...
- API 動作確認用のフロントエンド（`frontend/`）を拡張し、UI から操作できるようにする
...
### フロントエンド
- `frontend/`: API 動作確認用の UI 実装。`React + TypeScript + Vite` を使用。
- 主要機能: ログイン、ユーザー登録、ログインユーザー取得、ユーザー一覧取得、ユーザー削除、パスワード変更。
- 開発サーバー: `npm run dev`（`http://localhost:5173`）。
- API 接続: Vite のプロキシで `/api` を `http://localhost:8080` に転送。
